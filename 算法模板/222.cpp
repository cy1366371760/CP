...
struct gg{
    int fi, sc, num, l, r;// first mx  second mx  number of first mx
    ll sum = 0;
}seg[N << 2];
...
void push_up(int k)
{
    ...
    seg[k].fi = seg[ls].fi;
    seg[k].sc = seg[ls].sc;
    if(seg[rs].fi > seg[k].sc)seg[k].sc = seg[rs].fi;
    if(seg[k].sc > seg[k].fi)swap(seg[k].fi, seg[k].sc);
    if(seg[rs].sc > seg[k].sc)seg[k].sc = seg[rs].sc;
...
void upd(int L, int R, int val, int k = 1)
{
    ...
    {
        seg[k].sum += (val - seg[k].fi) * seg[k].num;
        seg[k].fi = val;
        return;
    }
...
int ask_mx(int L, int R, int k = 1)
{
